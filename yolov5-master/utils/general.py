...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device) # lcls表示分类损失，lbox表示框回归损失，lobj表示物体性损失
    tcls, tbox, indices, anchors = build_targets(p, targets, model)  # targets indices表示索引，anchors表示锚点
    h = model.hyp  # hyperparameters

    # Define criteria
    BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([h['cls_pw']])).to(device) # 初始化一个分类损失，一个候选框坐标损失
    BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([h['obj_pw']])).to(device)

    # Class label smoothing https://arxiv.org/pdf/1902.04103.pdf eqn 3
    cp, cn = smooth_BCE(eps=0.0) # 类别标签平滑处理
    ...
    # 针对三个预测特征层（P3, P4, P5）上的obj损失采用不同的权重。在源码中，针对预测小目标的预测特征层（P3）采用的权重是4.0，
    # 针对预测中等目标的预测特征层（P4）采用的权重是1.0，针对预测大目标的预测特征层（P5）采用的权重是0.4。
    balance = [4.0, 1.0, 0.4] if np == 3 else [4.0, 1.0, 0.4, 0.1]  # P3-5 or P3-6 根据输出结果的数量设置权重平衡系数
    for i, pi in enumerate(p):  # layer index, layer predictions i表示层索引，pi表示层预测值
        b, a, gj, gi = indices[i]  # image, anchor, gridy, gridx 从索引中获取图像、锚点、网格y坐标和网格x坐标。
        ...
        if n:
            ...
            pbox = torch.cat((pxy, pwh), 1).to(device)  # predicted box 计算预测框的坐标
            # 这里的tbox[i]中的xy是这个target对当前grid_cell左上角的偏移量[0,1]  而pbox.T是一个归一化的值
            iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, CIoU=True)  # iou(prediction, target) 计算预测框和目标框的IoU
            lbox += (1.0 - iou).mean()  # iou loss 计算回归损失。

            # Objectness 物体损失# self.gr是iou ratio [0, 1]  self.gr越大，置信度越接近iou  self.gr越小，置信度越接近1(人为加大训练难度)
...
#将计算得到的物体性损失乘以权重系数balance[i]，并累加到总的物体性损失lobj中。权重系数balance[i]用于对不同层的损失进行加权，可以根据需要调整不同层损失的重要性。
        lobj += BCEobj(pi[..., 4], tobj) * balance[i]  # obj loss 计算物体性损失，并根据权重加权。
...
def build_targets(p, targets, model):
    ...
    for i in range(det.nl):
        ...
        if nt:
            ...
            j = torch.max(r, 1. / r).max(2)[0] < model.hyp['anchor_t']  # compare # compare 宽度和高度方向差异最大的值与设定值的比较
